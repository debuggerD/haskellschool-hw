...
import Prelude (Show(..), Eq(..), ($), (.), flip, asTypeOf)
...
composition = \h ->
  case h of
    Left p_imp_q -> \p -> Left $ p_imp_q p
    Right p_imp_r -> \p -> Right $ p_imp_r p

-- Exercise 3 -----------------------------------------

transposition :: (p -> q) <-> (Not q -> Not p)
transposition = Conj forward backward
  where
    forward = modus_tollens
    backward nq_imp_np = \p -> 
      case double_negation of 
        Conj dnh _ -> 
          case double_negation of 
            Conj _ dnh2_backward -> dnh2_backward . (modus_tollens nq_imp_np) $ (dnh p)

-- Exercise 4 -----------------------------------------

de_morgan :: Not (p \/ q) <-> (Not p /\ Not q)
de_morgan = Conj forward backward
  where
    forward h = Conj (\p -> h (Left p)) (\q -> h (Right q))
    backward (Conj ha hb) = \p_or_q ->
      case p_or_q of
        Left p -> ha p
        Right q -> hb q
